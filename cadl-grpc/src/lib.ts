...
const { reportDiagnostic } = createCadlLibrary({
  ...
  diagnostics: {
    ...
    "root-operation": {
      ...
      messages: {
        default:
          "operations in the root namespace are not supported (no associated gRPC service)",
      },
    },
    ...
    "unsupported-input-type": {
      ...
      messages: {
        ...
        "wrong-type":
          "gRPC operations may only accept a named Model as an input",
      },
    },
    "unsupported-field-type": {
      severity: "error",
      messages: {
        unconvertible: paramMessage`cannot convert a ${"type"} to a proto3 type (only intrinsic types and models are supported)`,
        "unknown-intrinsic": paramMessage`no known proto3 scalar for intrinsic type ${"name"}`,
      },
    },
  },
  emitter: { names: ["grpc"] },
